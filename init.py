...
BUCKET_POSTFIX = '-halide-run'  # Gets put after the unix user ID to create the bucket name.
...
RUN \
  export LLVM_CONFIG=/tmp/clang/bin/llvm-config && \
...
def update_ecs_image():
    ...
    with cd('~/' + APP_NAME):
        run('docker build -t atilimcetin/halide .')
        #run('docker images')
        ...
        #print(login_str)
        #run('%s' % login_str)
        run('docker tag atilimcetin/halide ' + DOCKERHUB_TAG)
